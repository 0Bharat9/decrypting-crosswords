...
from crosswords.decryptor.models import answer_clues, setup_closedbook
...
class Solver:
    ...
    def get_candidates(self):
        ...
        matches = [(idx, r.search(clue))
                   for idx, clue in enumerate(all_clues) if r.search(clue) != None]
        for (idx, match) in matches:
            clue = all_clues[idx]
            var = str(match.group(1)) + str(match.group(2)[0]).upper()
            if var in self.crossword.variables:
                clue = clue[:match.start(
                )] + self.crossword.variables[var]['clue'] + clue[match.end():]
                all_clues[idx] = clue
        ...
        for index, var in enumerate(self.crossword.variables):
            ...
            self.candidates[var] = {"words": [],
                                    "bit_array": None, "weights": {}}
            ...
            if self.crossword.variables[var]["gold"] in words:
                print(clue, self.crossword.variables[var]["gold"], words.index(
            ...
            for word_idx, word in enumerate(self.candidates[var]["words"]):
                for pos_idx, char in enumerate(word):
                    ...
                    self.candidates[var]["bit_array"][char_idx,
                                                      pos_idx, word_idx] = 1
                    self.candidates[var]["single_query_cache"][pos_idx][char].append(
                        word)
                    self.candidates[var]["single_query_cache_indices"][pos_idx][char].append(
                        word_idx)
    ...
    def evaluate(self, solution):
        ...
        for var in self.crossword.variables:
            ...
            matching_cells = [self.crossword.letter_grid[cell[0]]
                              [cell[1]] == solution[cell[0]][cell[1]] for cell in cells]
            if len(cells) == sum(matching_cells):
                words_correct += 1
            else:
                print('evaluation: correct word', ''.join([self.crossword.letter_grid[cell[0]][cell[1]]
                      for cell in cells]), 'our prediction:', ''.join([solution[cell[0]][cell[1]] for cell in cells]))
            words_total += 1
        print("Letters Correct: {}/{} | Words Correct: {}/{}".format(int(letters_correct),
              int(letters_total), int(words_correct), int(words_total)))
        print("Letters Correct: {}% | Words Correct: {}%".format(
            float(letters_correct/letters_total*100), float(words_correct/words_total*100)))
