from crosswords.decryptor.solver.Utils import clean
...
class Crossword:
        ...
        for idx in range(len(answer)):
            ...
            if word_id in self.variables:
                ...
                self.variables[word_id] = {
                    "clue": clue, "gold": answer, "cells": [cell], "crossing": []}
    ...
    def initialize_crossing(self):
        for word_id in self.variables:
            ...
            crossing_ids = []
            for cell in cells:
                crossing_ids += list(filter(lambda x: x !=
                                     word_id, self.grid_cells[cell]))
                ...
                if self.number_grid[row][col] != "":
                    if cell_number in self.across:
                        self.initialize_variable(
                            (row, col), self.across, across=True)
                    if cell_number in self.down:
                        self.initialize_variable(
                            (row, col), self.down, across=False)
