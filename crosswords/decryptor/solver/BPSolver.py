...
from crosswords.decryptor.solver.Utils import print_grid, get_word_flips
from crosswords.decryptor.solver.Solver import Solver
from crosswords.decryptor.models import setup_t5_reranker, t5_reranker_score_with_clue

# our answer set
answer_set = set()
with open('crosswords/decryptor/checkpoints/biencoder/wordlist.tsv', 'r') as rf:
    for line in rf:
        w = ''.join([c.upper() for c in (line.split('\t')[-1]
                                         ).upper() if c in string.ascii_uppercase])
        answer_set.add(w)


# the probability of each alphabetical character in the crossword
UNIGRAM_PROBS = [('A', 0.0897379968935765), ('B', 0.02121248877769636), ('C', 0.03482206634145926), ('D', 0.03700942543460491), ('E', 0.1159773210750429), ('F', 0.017257461694024614), ('G', 0.025429024796296124), ('H', 0.033122967601502), ('I', 0.06800036223479956), ('J', 0.00294611331754349), ('K', 0.013860682888259786), ('L', 0.05130800574373874), ('M', 0.027962776827660175),
                 ('N', 0.06631994270448001), ('O', 0.07374646543246745), ('P', 0.026750756212433214), ('Q', 0.001507814175439393), ('R', 0.07080460813737305), ('S', 0.07410988246048224), ('T', 0.07242993582154593), ('U', 0.0289272388037645), ('V', 0.009153522059555467), ('W', 0.01434705167591524), ('X', 0.003096729223103298), ('Y', 0.01749958208224007), ('Z', 0.002659777584995724)]
...
LETTER_SMOOTHING_FACTOR = [0.0, 0.0, 0.04395604395604396, 0.0001372495196266813, 0.0005752186417796561, 0.0019841824329989103, 0.0048042463338563764, 0.013325257419745608, 0.027154447774285505, 0.06513517299341645,
                           0.12527790128946198, 0.22003002358996354, 0.23172376584839494, 0.254873006497342, 0.3985086992543496, 0.2764976958525346, 0.672645739910314, 0.6818181818181818, 0.8571428571428571, 0.8245614035087719, 0.8, 0.71900826446281, 0.0]
...
class BPVar:
    def __init__(self, name, variable, candidates, cells):
        ...
        self.ordered_cells = [cells_by_position[pos]
                              for pos in variable['cells']]
        self.candidates = candidates
        self.words = self.candidates['words']
        self.word_indices = np.array([[string.ascii_uppercase.index(
            l) for l in fill] for fill in self.candidates['words']])  # words x length of letter indices
        # the incoming 'weights' are costs
        self.scores = -np.array([self.candidates['weights'][fill]
    ...
    def _postprocess(self, all_letter_probs):
        ...
        for i in range(len(all_letter_probs)):
            all_letter_probs[i] = (1 - LETTER_SMOOTHING_FACTOR[self.length]) * \
                all_letter_probs[i] + \
                LETTER_SMOOTHING_FACTOR[self.length] * unigram_probs
    ...
    def propagate(self):
        ...
        for i in range(len(self.ordered_cells)):
            ...
            letter_probs = (self.candidates['bit_array'][:, i] *
                            np.expand_dims(word_probs, axis=0)).sum(axis=1) + 1e-8
            all_letter_probs.append(letter_probs)
        all_letter_probs = self._postprocess(
            all_letter_probs)  # unigram postprocessing
...
class BPCell:
    def __init__(self, position, clue_pair):
        ...
        self.log_probs = np.log(
            np.array([1./len(self.letters) for _ in range(len(self.letters))]))
        self.crossing_vars = []
        self.directional_scores = []
        self.prediction = {}
...
class BPSolver(Solver):
    ...
    def reset(self):
        ...
        for key, value in self.crossword.variables.items():
            var = BPVar(
                key, value, self.candidates[key], self.bp_cells_by_clue[key])
    ...
    def solve(self, num_iters=10, iterative_improvement_steps=5, return_greedy_states=False, return_ii_states=False):
        ...
        if return_greedy_states:
            grid, all_grids = self.greedy_sequential_word_solution(
                return_grids=True)
        else:
        ...
        for i in range(iterative_improvement_steps):
            ...
            self.evaluate(grid)
            grid, did_iterative_improvement_make_edit = self.iterative_improvement(
                grid)
    ...
    def get_candidate_replacements(self, uncertain_answers, grid):
        ...
        for clue in uncertain_answers.keys():
            ...
            clue_positions = [
                key for key, value in self.crossword.variables.items() if value['clue'] == clue]
            for clue_position in clue_positions:
                cells = sorted(
                    [cell for cell in self.bp_cells if clue_position in cell.crossing_clues], key=lambda c: c.position)
        ...
        for cell_id, cell in enumerate(self.bp_cells):
            ...
            new_characters = ['ABCDEFGHIJKLMNOPQRSTUVWXYZ'[i]
                              for i in range(26) if above_threshold[i]]
            # used = set()
            ...
            new_characters = [x for x in new_characters if x !=
        ...
        for i in range(len(candidate_replacements)):
            for j in range(i+1, len(candidate_replacements)):
                ...
                if flip1[0][0] != flip2[0][0]:
                    # shared clue
                    if len(set(flip1[0][0].crossing_clues + flip2[0][0].crossing_clues)) < 4:
        ...
        for cr in candidate_replacements:
            ...
            for var in variables:
                original_fill = ''.join(
                    [grid[cell.position[0]][cell.position[1]] for cell in var.ordered_cells])
                modified_fill = ''.join(
                    [modified_grid[cell.position[0]][cell.position[1]] for cell in var.ordered_cells])
        ...
        for var in self.crossword.variables:
            ...
            for cell in self.bp_cells:  # loop through all cells
                if cell.position in cells:  # if this cell is in the word we are currently handling
                    ...
                    cell.prediction[self.crossword.variables[var]
                                    ['clue']] = word
                    original_qa_pairs[self.crossword.variables[var]
                                      ['clue']] = word
    ...
    def score_grid(self, grid):
        ...
        for clue, cells in self.bp_cells_by_clue.items():
            letters = ''.join([grid[cell.position[0]][cell.position[1]]
    ...
    def greedy_sequential_word_solution(self, return_grids=False):
        ...
        cache = [(deepcopy(var.words), deepcopy(var.log_probs))
                 for var in self.bp_vars]
        ...
        for var in self.bp_vars:
            ...
            var.log_probs = var.log_probs + \
                math.log(1 - LETTER_SMOOTHING_FACTOR[var.length])
            ...
            for i, cell in enumerate(best_var.ordered_cells):
                ...
                for var in cell.crossing_vars:
                    if var != best_var:
                        ...
                        keep_indices = [j for j in range(
                            len(var.words)) if var.words[j][cell_index] == letter]
        ...
        for cell in self.bp_cells:
            if cell.position in unfilled_cells:
                grid[cell.position[0]][cell.position[1]
                                       ] = string.ascii_uppercase[cell.log_probs.argmax()]
    ...
    def iterative_improvement(self, grid):
        ...
        for replacements in self.candidate_replacements:
            ...
            for var in variables:
                ...
                modified_fill = ''.join(
                    [modified_grid[cell.position[0]][cell.position[1]] for cell in var.ordered_cells])
                clue_index = list(set(var.ordered_cells[0].crossing_clues).intersection(
                    *[set(cell.crossing_clues) for cell in var.ordered_cells]))[0]
                print('original:', original_fill, 'modified:', modified_fill)
                print('gold answer',
                      self.crossword.variables[clue_index]['gold'])
                print('clue', self.crossword.variables[clue_index]['clue'])
            print('original score:', original_grid_score,
                  'modified score:', modified_grid_score)
        ...
        if len(possible_edits) > 0:
            ...
            possible_edits = sorted(
                possible_edits, key=lambda x: x[1], reverse=True)
            selected_edits = []
            for edit in possible_edits:
                ...
                if len(variables_modified.intersection(variables)) == 0:
            ...
            for edit in selected_edits:
                ...
                for var in variables:
                    original_fill = ''.join(
                        [grid[cell.position[0]][cell.position[1]] for cell in var.ordered_cells])
                    modified_fill = ''.join(
                        [new_grid[cell.position[0]][cell.position[1]] for cell in var.ordered_cells])
                    print('original:', original_fill,
