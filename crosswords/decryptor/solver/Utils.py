...
dictionary = set([a.strip() for a in open(
    'crosswords/decryptor/solver/words_alpha.txt', 'r').readlines()])
...
def get_word_flips(fill, num_candidates=10):
    ...
    if min_length == 999:
        return [fill.upper()]
    all_results = sum([sorted(results[length], key=lambda x: -x[1])
                      for length in sorted(list(results.keys()))], [])
...
def convert_puz(fname):
    ...
    for row_idx in range(p.height):
        ...
        for col_index, item in enumerate(row_solution):
            if p.solution[cell + col_index:cell + col_index + 1] == '.':
                ...
                grid[row_idx][col_index] = [
                    "", row_solution[col_index: col_index + 1]]
    ...
    for clue in numbering.across:
        ...
        grid[int(clue['cell'] / p.width)][clue['cell'] %
                                          p.width][0] = str(clue['num'])
    ...
    mydict = {'metadata': {'date': None, 'rows': p.height, 'cols': p.width},
              'clues': {'across': across_clues, 'down': down_clues}, 'grid': grid}
    return mydict
