...
from crosswords.decryptor.DPR.dpr.models import init_biencoder_components
from crosswords.decryptor.DPR.dpr.options import setup_args_gpu, print_args, set_encoder_params_from_state
from crosswords.decryptor.DPR.dpr.indexer.faiss_indexers import DenseIndexer, DenseFlatIndexer
...
from crosswords.decryptor.DPR.dpr.utils.data_utils import Tensorizer
from crosswords.decryptor.DPR.dpr.utils.model_utils import load_states_from_checkpoint, get_model_obj

from transformers import GPT2LMHeadModel, GPT2Tokenizer, T5ForConditionalGeneration, AutoTokenizer
from crosswords.decryptor.segment_fill import segment_fill
...
def setup_closedbook(process_id):
    dpr = DPRForCrossword(
        "crosswords/decryptor/checkpoints/biencoder/dpr_biencoder.bin",
        "crosswords/decryptor/checkpoints/biencoder/wordlist.tsv",
        "crosswords/decryptor/checkpoints/biencoder/embeddings/embeddings.json_*",
...
def setup_t5_reranker(process_id):
    ...
    model = T5ForConditionalGeneration.from_pretrained(
        'crosswords/decryptor/checkpoints/byt5_reranker/')
    ...
    for clue, possibly_ungrammatical_fill in zip(clues, possibly_ungrammatical_fills):
        ...
        if clue + possibly_ungrammatical_fill in RERANKER_CACHE:
            ...
            with torch.inference_mode():
                inputs = tokenizer(
                    ['Q: ' + clue], return_tensors='pt')['input_ids'].to(model.device)
                labels = tokenizer([possibly_ungrammatical_fill], return_tensors='pt')[
...
def preprocess_clue_fn(clue):
    ...
    clue = ''.join(c for c in unicodedata.normalize(
        'NFD', clue) if unicodedata.category(c) != 'Mn')
    ...
    clue = re.sub(
        ": [Aa]bbreviat\.|: [Aa]bbrev\.|: [Aa]bbrv\.|: [Aa]bbrv|: [Aa]bbr\.|: [Aa]bbr", " abbreviation", clue)
...
def answer_clues(dpr, clues, max_answers, output_strings=False):
    ...
    outputs = dpr.answer_clues_closedbook(
        clues, max_answers, output_strings=output_strings)
    return outputs
    ...
    def generate_question_vectors(self, questions: List[str]) -> T:
        ...
        with torch.no_grad():
            for j, batch_start in enumerate(range(0, n, bsz)):
                ...
                q_ids_batch = torch.stack(
                    batch_token_tensors, dim=0).to(self.device)
...
class DPRForCrossword(object):
    ...
    def __init__(
        ...
        # am I a wikipedia retrieval model or a closed-book model
        ...
        self.device = torch.device(
            "cuda:"+str(process_id % torch.cuda.device_count()))
        ...
        if not retrievalmodel:
            ...
            for my_id in self.all_passages.keys():
                temp[my_id] = "".join(
                    [c.upper() for c in self.all_passages[my_id][1] if c.upper() in string.ascii_uppercase])
    ...
    def answer_clues_closedbook(self, questions, max_answers, output_strings=False):
        ...
        top_ids_and_scores = self.retriever.get_top_docs(
            questions_tensor.numpy(), max_answers)
    ...
    def get_wikipedia_docs(self, questions, max_docs):
        ...
        top_ids_and_scores = self.retriever.get_top_docs(
            questions_tensor.numpy(), max_docs + 2)
