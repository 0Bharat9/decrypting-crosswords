...
from ..data.reader_data import ReaderSample, ReaderPassage
...
def _create_question_passages_tensors(positives: List[ReaderPassage], negatives: List[ReaderPassage], total_size: int,
    ...
    if is_train:
        ...
        answer_mask = torch.zeros(
            (total_size, max_n_answers), dtype=torch.long)
    ...
    negatives_selected = [_pad_to_len(
        negatives[i].sequence_ids, pad_token_id, max_len) for i in negative_idxs]

    while len(negatives_selected) < total_size - positives_num:
        negatives_selected.append(empty_ids.clone())

    input_ids = torch.stack(
        [t for t in positives_selected + negatives_selected], dim=0)
