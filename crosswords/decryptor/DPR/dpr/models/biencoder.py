...
from ..utils.data_utils import normalize_question
...
class BiEncoder(nn.Module):
    ...
    def create_biencoder_input(
        ...
        for sample in samples:
            ...
            if do_lower_fill:
                neg_ctxs, hard_neg_ctxs = list(map(lambda x: {"text": x["text"].lower(), "title": x["title"]}, neg_ctxs)), list(
                    map(lambda x: {"text": x["text"].lower(), "title": x["title"]}, hard_neg_ctxs))
        ...
        ctxs_tensor = torch.cat([ctx.view(1, -1)
                                for ctx in ctx_tensors], dim=0)
        questions_tensor = torch.cat([q.view(1, -1)
                                     for q in question_tensors], dim=0)
