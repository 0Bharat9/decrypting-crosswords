...
def convert_retriever_results(
    ...
    logger.info(
        "Loaded %d questions + retrieval results from %s", len(
            samples), input_file
    )
...
def get_best_spans(
    ...
    for (start_index, end_index), score in scores:
        ...
        predicted_answer = tensorizer.to_string(
            ctx_ids[start_index: end_index + 1]).upper()
        predicted_answer = ''.join(
            [r.upper() for r in predicted_answer if r.upper() in string.ascii_uppercase])
...
def _select_reader_passages(
    ...
    ctxs = [ReaderPassage(**ctx)
            for ctx in sample["ctxs"]][0:max_retriever_passages]
    answers_token_ids = [
        ...
        if ctx.has_answer:
            ...
            answer_spans = [
                _find_answer_positions(
                    ctx.passage_token_ids, answers_token_ids[i])
                for i in range(len(answers))
            ]

            # flatten spans list
            answer_spans = [
                item for sublist in answer_spans for item in sublist]
    ...
    if include_gold_passage and question in gold_passage_map:
        ...
        if not included_gold_passage:
            ...
            if not gold_passage.has_answer:
                logger.warning(
                    "No answer found in gold passage %s", gold_passage)
            else:
...
def _get_gold_ctx_dict(file: str) -> Tuple[Dict[str, ReaderPassage], Dict[str, str]]:
    ...
    # question from tokens -> original question (NQ only)
    original_questions = {}
